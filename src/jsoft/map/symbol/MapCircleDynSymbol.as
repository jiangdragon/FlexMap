package jsoft.map.symbol
{
	...
	public class MapCircleDynSymbol extends Symbol
	{
		...
		public function MapCircleDynSymbol() {
		}
		...
		public override function copyTo(symbol:Symbol):void {
			super.copyTo(symbol);
			var point:MapCircleDynSymbol = symbol as MapCircleDynSymbol;
			point.color = color;
		...
		private function drawSinglePoint(geometry:Geometry):void {
			...
			//AppContext.getAppUtil().alert("drawSinglePoint viewSize="+viewSize);
			...
			var cx:int = viewSize;
			var cy:int = viewSize;
			var width:int = viewSize * 2;
			var height:int = viewSize * 2;
