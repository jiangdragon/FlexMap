package jsoft.map.vector.poly
{
	...
	public class CircleVector extends BaseVector implements IVector
	{
		...
		public override function clone():IVector {
			var newVector:CircleVector = new CircleVector();
			copyTo(newVector);
		...
		private function drawSinglePolygon(geometry:Geometry,coordinate:Coordinate,pointCount:int):int {
			...
			var xLength:Number = x_arr[0]-x_arr[2];
			var yLength:Number = y_arr[0]-y_arr[2];
			var radius:int = Math.sqrt(xLength * xLength + yLength * yLength);
			drawUtil.drawCircle(x_arr[0],y_arr[0],radius);
			return pointCount;
		}
