package jsoft.map.content
{
	...
	public class MapTile extends UIComponent implements ImageReceiver
	{
		...
		public function MapTile(level:int,tileX:int,tileY:int,tileURL:String,bitmap:BitmapData=null,errorImage:MapTile=null) {
			super();
			this.level = level;
			this.tileX = tileX;
			this.tileY = tileY;
			...
			if(bitmap != null) {
				...
				var image:ImageReader = new ImageReader(tileURL);
				image.addDataRecv(this);
			}
			addEventListener(Event.RENDER,onRender);
			//addEventListener(MouseEvent.MOUSE_WHEEL,AppContext.getMapContext().getMapContent().onMouseWheel);
		}
